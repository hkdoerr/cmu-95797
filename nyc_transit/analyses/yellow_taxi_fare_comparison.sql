...
FROM  (select * from {{ ref('stg__yellow_tripdata') }} limit 10000  --limiting to 10k records due to memory constriants
    ) yellow
    LEFT JOIN  {{ ref('mart__dim_locations') }} loc on yellow.PUlocationID = loc.LocationID
