...
from {{ ref('mart__fact_all_taxi_trips') }} a
    left join {{ ref('mart__dim_locations') }} start_loc on a.PUlocationID = start_loc.LocationID
    left join {{ ref('mart__dim_locations') }} end_loc on a.DOlocationID = end_loc.LocationID) x
group by all